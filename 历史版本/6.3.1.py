...
LEFT_CONFIG = {
    ...
    'max_extension_angle': 15,    # 最大延长角度(度)
}
...
def extend_line_segment(line, img_shape, config):
    """
    延长线段以寻找可能的连接
    Args:
        line: 输入线段点集
    ...
    # 计算延长长度
    extend_length = min(config['min_extension_length'], 
                       config['max_gap_for_matching'] * 1.5)
    
    # 延长起点和终点
    new_start = start_pt - direction * extend_length
    new_end = end_pt + direction * extend_length
    
    # 确保延长点在图像范围内
    h, w = img_shape[:2]
    new_start = np.clip(new_start, [0, 0], [w-1, h-1])
    new_end = np.clip(new_end, [0, 0], [w-1, h-1])
    
    # 创建延长后的线段
    extended_line = np.vstack([new_start, line, new_end])
    return extended_line

def find_line_intersections(line1, line2):
    """
    计算两条线段的交点
    Args:
        line1: 第一条线段点集
...
def match_broken_lines(lines, img, config):
    ...
    for line in lines:
        ...
        if feat is not None and feat['length'] > config['min_line_length']/2:
    ...
    # 按x坐标排序（从左到右）
    sorted_indices = np.argsort([f['start_point'][0] for f in features])
    ...
    for i in range(len(features)):
        ...
        # 延长当前线段
        extended_line = extend_line_segment(current_line, img.shape, config)
        ...
        for j in range(i+1, len(features)):
            ...
            if dist > config['max_gap_for_matching'] * 1.5:
                continue
                
            # 延长另一条线段
            candidate_line = valid_lines[j]
            extended_candidate = extend_line_segment(candidate_line, img.shape, config)
            
            # 计算交点
            intersection = find_line_intersections(extended_line, extended_candidate)
            ...
            if intersection is not None:
                ...
                # 曲率相似性
                curv_diff = abs(current_feat['curvature'] - features[j]['curvature'])
                
                # 综合匹配条件
                if (dir_sim > config['direction_similarity'] and 
                    inten_diff < (1 - config['intensity_similarity']) and 
                    curv_diff < 0.1):
                    labels[j] = current_label
                    current_feat = features[j]  # 更新当前特征为最新匹配的线段
                    current_line = valid_lines[j]
                    extended_line = extend_line_segment(current_line, img.shape, config)
