...
def stereo_match(left_lines, right_lines, left_img, right_img, config, Q):
    ...
    for left_idx, right_idx, score in matches:
        ...
        point_4d_homogeneous = cv2.perspectiveTransform(point_3d[None, None], Q)  # 添加两个维度变成 (1, 1, 3)
        depth = point_4d_homogeneous[0, 0, 2]  # 提取深度值
...
def visualize_results(img, lines, title):
    ...
    for line in lines:
        # 生成三通道颜色（确保整数类型）
        color_index = (line['label'] - 1) % 20  # 从0开始索引tab20颜色
        color_rgba = cmap(color_index)
        color = tuple(int(c * 255) for c in color_rgba[:3])  # 转换为BGR三元组
...
def visualize_matches(left_img, right_img, matches):
    ...
    # 使用Matplotlib传统方式获取颜色映射
    cmap = plt.cm.get_cmap('tab20')  # 替代已弃用的 colormaps() 方法
    
    # 绘制左图
    left_vis = left_img.copy()
    for i, match in enumerate(matches):
        # 生成三通道颜色（确保整数类型）
        color_index = i % 20  # 从0开始索引tab20颜色
        color_rgba = cmap(color_index)
        color = tuple(int(c * 255) for c in color_rgba[:3])  # 转换为BGR三元组
