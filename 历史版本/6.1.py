...
LEFT_CONFIG = {
    ...
    # 新增全局中值滤波参数
    'median_kernel_size': 5,     # [5-25] 中值滤波核大小，值越大背景抑制越强
    
    # 预处理参数
    'clahe_clip': 2.5,           # [1.0-5.0] 对比度受限直方图均衡化的对比度上限
    'blur_kernel': (7, 7),       # [(3,3)-(9,9)] 高斯模糊核大小
    'gamma_correct': 1.0,        # [0.3-1.0] 伽马校正系数
    'specular_thresh': 200,      # [150-250] 高光检测阈值
    ...
    # 质心检测
    'dynamic_thresh_ratio':0.6, # [0.2-0.6] 动态阈值比例
    'min_line_width': 1,         # [3-10] 最小有效线宽（像素）
    'max_line_gap': 50,           # 【减小间隙容忍度】

    # 几何约束
    'roi_padding': 10,           # [0-100] 边缘裁剪宽度
    'cluster_eps': 24,           # 聚类半径
    'min_samples': 8,           # [5-20] 最小聚类点数
    'min_line_length': 300,       # [30-100] 有效线段最小长度

    # 后处理
    'smooth_sigma': 2.5,         # 【适中平滑】高斯平滑强度
    'max_end_curvature': 0.1,    # [0.1-0.5] 端点最大允许曲率

    'smooth_degree': 2.5,        # 适中的插值平滑度
    
    # 新增阴影去除参数
    'shadow_region': (2/3, 1),   # 处理图像下侧1/3区域
...
def remove_shadow_region(img, config):
    ...
    # 应用中值滤波
    median = cv2.medianBlur(bottom_region, config['median_kernel_size'])
...
if __name__ == "__main__":
    left_img = cv2.imread('22.1.bmp')
    right_img = cv2.imread('22.0.bmp')
