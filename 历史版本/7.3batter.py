...
def stereo_rectify(left_img, right_img):
    ...
    R1, R2, P1, P2, Q, validPixROI1, validPixROI2 = cv2.stereoRectify(
        ...
        (w, h), R, T, flags=cv2.CALIB_ZERO_DISPARITY, alpha=1.0  # 修改alpha为1.0保持原始尺寸
    )
...
def rectify_lines(lines, R, img_size):
    ...
    for line in lines:
        ...
        # 应用旋转矩阵变换
        rect_points = (R @ homogeneous.T).T[:, :2]
...
def find_candidate_matches(right_line, left_lines, config, baseline, focal_length, Q):
    """
    在左图中寻找候选匹配线段（以右图为基准）
    Args:
        right_line: 右图线段
    ...
    for idx, left_line in enumerate(left_lines):
        ...
        # 检查视差范围
        disparity = left_mid[0] - right_mid[0]  # 左图x - 右图x
        if min_disp <= disparity <= max_disp:
            candidates.append(idx)
    
    return candidates

def evaluate_match_quality(right_line, left_line, right_img, left_img, config):
    """
    评估匹配质量（以右图为基准）
    Args:
        right_line: 右图线段
    ...
    overlap_start = max(right_y_range[0], left_y_range[0])
    overlap_end = min(right_y_range[1], left_y_range[1])
    
    if overlap_end <= overlap_start:
        return 0
        
    overlap_ratio = (overlap_end - overlap_start) / (right_y_range[1] - right_y_range[0])
    
    # 计算方向相似度
    right_dir = right_line['features']['direction']
    left_dir = left_line['features']['direction']
    dir_similarity = abs(right_dir @ left_dir)
    ...
    if left_width == 0:
        ...
        width_ratio = right_width / left_width
...
def stereo_match(right_lines, left_lines, right_img, left_img, config, Q):
    """
    立体匹配主函数（以右图为基准）
    Args:
    ...
    # 统计待匹配线段数量
    print(f"右图待匹配线段数量: {len(right_lines)}")
    print(f"左图候选线段数量: {len(left_lines)}")
    
    matches = []
    matched_left_indices = set()
    
    # 对右图线段按x坐标排序
    right_lines_sorted = sorted(right_lines, key=lambda x: np.mean(x['points'][:,0]))
    
    # 计算相邻线段的期望宽度差
    if len(right_lines_sorted) > 1:
        x_positions = [np.mean(line['points'][:,0]) for line in right_lines_sorted]
        avg_width_diff = np.mean(np.diff(x_positions))
    else:
        avg_width_diff = 0
    
    # 对每条右图线段寻找最佳匹配
    for right_idx, right_line in enumerate(right_lines_sorted):
        candidates = find_candidate_matches(right_line, left_lines, config, baseline, focal_length, Q)
        
        # 输出当前右图线段的候选数量
        print(f"右图线段 {right_idx+1} 在左图中有 {len(candidates)} 条候选匹配线段")
        ...
        for left_idx in candidates:
            ...
            # 计算基本匹配分数
            score = evaluate_match_quality(right_line, left_lines[left_idx], right_img, left_img, config)
            
            # 添加宽度均匀性约束
            if avg_width_diff > 0:
                expected_x = np.mean(right_line['points'][:,0]) + right_idx * avg_width_diff
                actual_x = np.mean(left_lines[left_idx]['points'][:,0])
                x_diff = abs(expected_x - actual_x)
                width_consistency = max(0, 1 - x_diff / (avg_width_diff * 2))  # 允许两倍平均宽度差的偏差
                score = score * 0.7 + width_consistency * 0.3  # 调整权重
            
            if score > best_score:
                best_score = score
                best_left_idx = left_idx
        
        if best_left_idx != -1 and best_score > 0:
            matches.append((right_idx, best_left_idx, best_score))
    ...
    # 构建结果
    results = []
    for right_idx, left_idx, score in matches:
        right_line = right_lines_sorted[right_idx]
        ...
        # 对右图线段的每个点，在左图线段上找到最近的点计算视差
        for right_pt in right_line['points']:
            # 在左图线段上找到y坐标最接近的点
            left_pts = left_line['points']
            y_dists = np.abs(left_pts[:,1] - right_pt[1])
            closest_idx = np.argmin(y_dists)
            left_pt = left_pts[closest_idx]
            
            disparity = left_pt[0] - right_pt[0]  # 左图x - 右图x
            disparities.append(disparity)
            
            # 计算深度
            point_3d = np.array([[[right_pt[0], right_pt[1], disparity]]], dtype=np.float32)
            try:
        ...
        results.append({
            ...
            'right_width': calculate_line_width(right_line['points'], right_img),
            'left_width': calculate_line_width(left_line['points'], left_img),
            'right_index': right_idx,  # 右图线段的排序索引
            'candidate_count': len(find_candidate_matches(right_line, left_lines, config, baseline, focal_length, Q))  # 候选匹配数量
        })
    
    # 按右图线段顺序排序结果
    results.sort(key=lambda x: x['right_index'])
    
    # 计算宽度均匀性指标
    if len(results) > 1:
        x_positions = [np.mean(res['right_line']['points'][:,0]) for res in results]
...
def visualize_matches(right_img, left_img, matches):
    """
    可视化匹配结果（改进版本）
    Args:
        right_img: 右图
    ...
    for i, match in enumerate(matches):
        ...
        cv2.putText(right_vis, str(i+1), tuple(pts[0]), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
    ax1.imshow(cv2.cvtColor(right_vis, cv2.COLOR_BGR2RGB))
    ax1.set_title('Right Image with Matched Lines')
    ...
    ax2.imshow(cv2.cvtColor(left_vis, cv2.COLOR_BGR2RGB))
    ax2.set_title('Left Image with Matched Lines')
...
if __name__ == "__main__":
    ...
    # 矫正线段坐标（使用旋转矩阵）
    print("矫正线段坐标...")
    img_size = (left_img.shape[1], left_img.shape[0])  # (w, h)
    left_lines_rectified = rectify_lines(left_lines, R1, img_size)
    right_lines_rectified = rectify_lines(right_lines, R2, img_size)

    # 进行立体匹配（以右图为基准）
    print("\n进行立体匹配（以右图为基准）...")
    matches = stereo_match(right_lines_rectified, left_lines_rectified, right_rectified, left_rectified, RIGHT_CONFIG, Q)
    ...
    # 可视化匹配结果（在矫正后的图像上）
    visualize_matches(right_rectified, left_rectified, matches)
    
    # 保存匹配结果
    print("\n匹配结果:")
    for i, match in enumerate(matches):
        print(f"匹配对 {i+1}:")
        print(f"  右图线段 {match['right_line']['label']} <-> 左图线段 {match['left_line']['label']}")
        print(f"  匹配分数: {match['match_score']:.2f}, 视差: {match['disparity']:.2f}px, 深度: {match['depth']:.2f}mm")
        print(f"  右图宽度: {match['right_width']:.2f}px, 左图宽度: {match['left_width']:.2f}px")
        print(f"  候选匹配数量: {match['candidate_count']}")
