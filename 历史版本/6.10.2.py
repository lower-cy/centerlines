...
LEFT_CONFIG = {
    ...
    # 高斯拟合参数
    'window_size': 15,           # 拟合窗口大小（单侧）
    'fit_threshold': 30,         # 最小强度阈值
    'max_fit_iterations': 50,    # 最大拟合次数
}
...
def fit_gaussian_1d(intensity_profile, config):
    """
    对强度剖面进行高斯拟合
    Args:
        intensity_profile: 一维强度数组
        config: 配置参数
    Returns:
        拟合成功返回(亚像素中心, 拟合参数)，失败返回None
    """
    if np.max(intensity_profile) < config['fit_threshold']:
        return None
    
    x = np.arange(len(intensity_profile))
    initial_guess = (
        np.max(intensity_profile) - np.min(intensity_profile),  # 幅值
        np.argmax(intensity_profile),  # 中心位置
        2.0,  # sigma
        np.min(intensity_profile)  # 偏移量
    )
    ...
    try:
        popt, pcov = curve_fit(
            ...
            intensity_profile, 
            p0=initial_guess,
            maxfev=config['max_fit_iterations']
        )
        # 检查拟合质量
        if 0 <= popt[1] <= len(intensity_profile) and popt[2] > 0.5:
            return popt[1], popt
    except:
        pass
...
def refine_line_with_gaussian_fit(line, original_img, config):
    """
    使用高斯拟合优化中心线位置（亚像素级）
    Args:
        line: 原始中心线点集 (N,2)
    ...
    for pt in line:
        ...
        x_end = min(gray.shape[1], x + window_size + 1)
        profile = gray[y, x_start:x_end]
        
        # 高斯拟合
        fit_result = fit_gaussian_1d(profile, config)
        if fit_result:
            subpixel_x = x_start + fit_result[0]
            refined_line.append([subpixel_x, y])
        else:
            refined_line.append([x, y])  # 拟合失败则使用原始坐标
    
    return np.array(refined_line)

def refine_all_lines_with_gaussian(lines, original_img, config):
    """
    对所有检测到的线段进行高斯拟合优化
    Args:
        lines: 检测到的线段列表
    ...
    refined_lines = []
    for line in lines:
        refined_points = refine_line_with_gaussian_fit(line['points'], original_img, config)
        refined_lines.append({
            'label': line['label'],
            'points': refined_points,
            'features': line['features']  # 注意：特征需要重新计算
        })
...
if __name__ == "__main__":
    ...
    refined_left_lines = refine_all_lines_with_gaussian(left_lines, left_img, LEFT_CONFIG)
    unique_left_labels = set(line['label'] for line in refined_left_lines) if refined_left_lines else set()
    print(f"左图提取到 {len(unique_left_labels)} 条中心线（共 {len(refined_left_lines)} 个线段）")

    print("\n处理右图(R.jpg)...")
    right_lines = detect_laser_lines(right_img, RIGHT_CONFIG)
    refined_right_lines = refine_all_lines_with_gaussian(right_lines, right_img, RIGHT_CONFIG)
    unique_right_labels = set(line['label'] for line in refined_right_lines) if refined_right_lines else set()
    print(f"右图提取到 {len(unique_right_labels)} 条中心线（共 {len(refined_right_lines)} 个线段）")

    visualize_results(left_img, refined_left_lines, 'Left Image')
    visualize_results(right_img, refined_right_lines, 'Right Image')

    save_labeled_lines(refined_left_lines, 'left_labeled_lines.csv')
    save_labeled_lines(refined_right_lines, 'right_labeled_lines.csv')
