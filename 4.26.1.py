...
ENHANCED_CONFIG = {
    ...
    'closure_kernel_L': (15,15),        # 左图闭运算核
    'closure_kernel_R': (25,25),        # 右图闭运算核
    'mask_ratio_L': 0.65,               # 左图掩膜阈值
    'mask_ratio_R': 0.4,                # 右图掩膜阈值

    # 新增三维重建相关参数
    'baseline': 0.07,          # 相机基线距离（米）
    'focal_length': 0.0035,    # 焦距（米）
    'pixel_size': 5.2e-6,      # 像素尺寸（米）
    'depth_min': 0.2,          # 最小深度（米）
    'depth_max': 0.8,          # 最大深度（米）
    'y_tolerance': 20          # 垂直方向容忍度（像素）
}
...
def calculate_disparity_range(config):
    """根据配置计算视差范围"""
    d_min = (config['baseline'] * config['focal_length']) / config['depth_max']
    d_max = (config['baseline'] * config['focal_length']) / config['depth_min']
    
    d_min_px = d_min / config['pixel_size']
    d_max_px = d_max / config['pixel_size']
    
    return max(1, int(d_min_px)), int(d_max_px)

def match_lines(left_lines, right_lines, config):
    """基于深度约束的左右线匹配"""
    d_min, d_max = calculate_disparity_range(config)
    ...
    for l_idx, l_line in enumerate(left_sorted):
        ...
        for r_idx, r_line in enumerate(right_sorted):
            ...
            # 垂直方向检查
            if abs(l_center_y - r_center_y) > config['y_tolerance']:
                continue
            ...
            # 检查是否在允许的视差范围内
            if d_min <= disparity <= d_max:
                if disparity < best_r_dist:
...
def visualize_matches(left_img, right_img, matched_pairs, thickness=2):
    """可视化匹配结果"""
    vis_left = cv2.cvtColor(left_img, cv2.COLOR_BGR2RGB).copy()
    vis_right = cv2.cvtColor(right_img, cv2.COLOR_BGR2RGB).copy()
    ...
    return np.hstack((vis_left, vis_right))
...
if __name__ == "__main__":
    try:
        ...
        raise FileNotFoundError("请确保L.bmp和R.jpg存在于当前目录且可读取")
    ...
    # 新增深度约束匹配
    print("正在执行深度约束匹配...")
    matched_pairs = match_lines(left_lines, right_lines, ENHANCED_CONFIG)  # 传递配置参数
    ...
    ax7.set_title('Depth-Constrained Matching')
