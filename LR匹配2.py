...
CAMERA_PARAMS = {
    ...
    'min_depth': 0,                    # 最小有效深度(mm)
    'max_depth': 0,                   # 最大有效深度(mm)
    'max_disparity': 0,                # 最大视差(像素)
...
E = np.array([
    ...
    [-0.34266487569182286, -65.93100047369474, 0.6149425234986867]
])
...
LEFT_CONFIG = {
    ...
    'min_laser_intensity':100,   # [30-100] 最低有效激光强度，值越大过滤弱光点越多
    ...
    'specular_thresh':145,      # [150-250] 高光检测阈值，值越小更多区域被视为高光
    ...
    # 新增阴影去除参数
    'shadow_region': (3/4, 1),   # 处理图像下1/3区域
...
RIGHT_CONFIG = {
    ...
    'min_line_width': 3,         # 允许更细的激光线
    'max_line_gap': 150,          # 【断裂容忍度】
    ...
    'max_end_curvature': 0.2,    # 的曲率限制，消除毛刺
    'smooth_degree': 2.5,        # 插值平滑度
    
    # 新增阴影去除参数
    'shadow_region': (4/5, 1),   # 处理图像下1/3区域
    'median_blur_size': 21,      # 中值滤波核大小
    'shadow_threshold': 30,      # 阴影阈值
}
...
def match_left_right_lines(left_lines, right_lines, img_shape):
    """
    基于极线约束和深度限制的左右线匹配
    Args:
        left_lines: 左图像检测到的激光线
    ...
    for left_line in left_lines:
        for right_line in right_lines:
            ...
            try:
                ...
                # 极线约束检查
                right_epilines = compute_epilines(left_overlap[:, :2], 1)
                valid_disparities = []
                
                for i, right_point in enumerate(right_overlap):
                    if is_point_on_epiline(right_point[:2], right_epilines[i]):
                        disparity = left_points[i,0] - right_point[0]
                        if 0 < disparity < CAMERA_PARAMS['max_disparity']:
                ...
                if np.std(valid_disparities) > CAMERA_PARAMS['disparity_threshold']:
                    continue
...
def compute_epilines(points, which_image):
    """
    计算对应极线（基于基础矩阵F）
    Args:
        points: 点集 (Nx2)
        which_image: 1表示左图点计算右图极线，2表示右图点计算左图极线
    Returns:
        极线方程参数 (Nx3)
    """
    homogeneous_points = np.column_stack([points, np.ones(len(points))])
    if which_image == 1:
        lines = (F @ homogeneous_points.T).T  # 左图点 -> 右图极线
    else:
        lines = (F.T @ homogeneous_points.T).T  # 右图点 -> 左图极线
    return lines

def is_point_on_epiline(point, line, tolerance=1.0):
    """
    判断点是否在极线上（带容差范围）
    Args:
        point: (x, y) 坐标
        line: 极线参数 (a, b, c) 表示 ax+by+c=0
        tolerance: 容许误差（像素）
    Returns:
        bool 是否在极线上
    """
    x, y = point
    a, b, c = line
    distance = abs(a*x + b*y + c) / np.sqrt(a**2 + b**2 + 1e-6)
...
if __name__ == "__main__":
    ...
    # 匹配左右图像中的激光线
    matched_pairs = match_left_right_lines(left_lines, right_lines, left_img.shape)  # ✅ 参数正确
    ...
    # 可视化结果
    visualize_results(left_img, left_lines, 'Left Image', left_depth_info)
