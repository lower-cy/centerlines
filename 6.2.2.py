...
def calculate_similarity(seg1, seg2):
    """
    计算两个线段的相似度（位置+方向）
    返回 (distance, angle_diff, is_similar)
    """
    # 获取端点
    p1_start, p1_end = seg1[0], seg1[-1]
    p2_start, p2_end = seg2[0], seg2[-1]
    
    # 计算最近端点距离
    dist1 = np.linalg.norm(p1_end - p2_start)
    dist2 = np.linalg.norm(p1_start - p2_end)
    ...
    if norm1 == 0 or norm2 == 0:
        return min_dist, np.pi, False
        
    cos_angle = np.dot(vec1, vec2) / (norm1 * norm2)
    angle_diff = np.arccos(np.clip(cos_angle, -1.0, 1.0))
    
    # 综合判断
    config = LEFT_CONFIG if len(seg1) > 0 and seg1[0][0] < 1000 else RIGHT_CONFIG
    similar = (min_dist <= config['max_gap_to_connect'] and 
              abs(angle_diff) <= np.arccos(config['angle_similarity']))
    
    return min_dist, angle_diff, similar
...
def connect_broken_lines(lines, img_shape, config):
    ...
    for i in range(n):
        for j in range(i+1, n):
            dist, angle_diff, similar = calculate_similarity(lines[i], lines[j])
            if similar:
                adj[i].append(j)
                adj[j].append(i)
    ...
    for i, label in enumerate(labels):
        components[label].append(lines[i])
    
    # 阶段4：整合每个连通分量
    connected_lines = []
    for label in sorted(components.keys()):
        segments = components[label]
        
        # 合并所有点
        all_points = np.vstack([seg for seg in segments])
        
        # 去重
        _, idx = np.unique(all_points, axis=0, return_index=True)
        unique_points = all_points[np.argsort(idx)]
        
        # 按y坐标排序
        sorted_points = unique_points[unique_points[:,1].argsort()]
        
        # 样条插值
        if len(sorted_points) >= 4:
            try:
                tck, _ = splprep(sorted_points.T, s=config['smooth_degree']*2)
                u = np.linspace(0, 1, int(len(sorted_points)*2))
                interpolated = np.column_stack(splev(u, tck))
                connected_lines.append(interpolated)
            except:
                connected_lines.append(sorted_points)
        else:
            connected_lines.append(sorted_points)
