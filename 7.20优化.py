...
def find_all_candidate_matches(left_lines, right_lines, config, baseline, focal_length, Q):
    """
    为右图每条线寻找左图中所有可能的匹配候选（在深度范围内）
    Args:
        left_lines: 左图线段列表
        right_lines: 右图线段列表
        config: 配置参数
        ...
        Q: 重投影矩阵
    Returns:
        字典，键为右图线段索引，值为候选匹配列表，每个候选包含左图索引和深度
    """
    ...
    for right_idx, right_line in enumerate(right_lines):
        ...
        # 如果没有候选，则使用左图第一条线计算深度（仅用于调试）
        if not candidates:
            left_mid = np.mean(left_lines[0]['points'], axis=0)
            disparity = left_mid[0] - right_mid[0]
            point_3d = np.array([[[left_mid[0], left_mid[1], disparity]]], dtype=np.float32)
            try:
                point_4d = cv2.perspectiveTransform(point_3d, Q)
        ...
        candidates_dict[right_idx] = candidates
    
    return candidates_dict

def evaluate_width_uniformity(matches, left_lines, right_lines):
    """
    评估宽度均匀性并返回最佳匹配组合
    Args:
        matches: 所有候选匹配字典
    ...
    # 生成所有可能的匹配组合
    all_combinations = product(*[ [(right_idx, cand) for cand in cands] 
                                for right_idx, cands in matches.items() ])
    ...
    for combination in all_combinations:
        # 提取左图索引并按x坐标排序
        sorted_comb = sorted(combination, key=lambda x: x[1]['left_mid'][0])
        left_indices = [x[1]['left_idx'] for x in sorted_comb]
        
        # 计算相邻线段的x坐标差
        if len(left_indices) > 1:
            x_positions = [np.mean(left_lines[idx]['points'][:,0]) for idx in left_indices]
            diffs = np.diff(x_positions)
            width_std = np.std(diffs)
            
            if width_std < best_score:
                best_score = width_std
...
def stereo_match(left_lines, right_lines, left_img, right_img, config, Q):
    ...
    # 1. 为右图每条线寻找左图中所有可能的匹配候选
    print("\n为右图每条线寻找左图中所有可能的匹配候选...")
    all_candidates = find_all_candidate_matches(left_lines, right_lines, config, baseline, focal_length, Q)
    
    # 打印所有候选匹配的深度信息
    print("\n右图每条线在深度范围内对应左图每条线的深度:")
    for right_idx, candidates in all_candidates.items():
        print(f"\n右图线段 {right_idx+1} 的候选匹配:")
        for cand in candidates:
            print(f"  左图线段 {cand['left_idx']+1}: 深度={cand['depth']:.2f}mm, 视差={cand['disparity']:.2f}px")
    
    # 2. 评估宽度均匀性并选择最佳匹配组合
    print("\n评估宽度均匀性并选择最佳匹配组合...")
    best_combination, width_std = evaluate_width_uniformity(all_candidates, left_lines, right_lines)
    
    # 3. 构建最终匹配结果
    results = []
    if best_combination:
        print(f"\n最佳匹配组合的宽度标准差: {width_std:.2f}px")
        
        for right_idx, cand in best_combination:
            left_idx = cand['left_idx']
            left_line = left_lines[left_idx]
            right_line = right_lines[right_idx]
            
            # 计算匹配质量分数
            score = evaluate_match_quality(left_line, right_line, left_img, right_img, config)
...
def visualize_matches(left_img, right_img, matches, all_candidates=None):
    """
    改进的可视化匹配结果（显示所有候选匹配）
    Args:
        left_img: 左图
    ...
    if matches:
        x_positions = [np.mean(match['left_line']['points'][:,0]) for match in matches]
        ...
        if len(x_positions) > 1:
            diffs = np.diff(x_positions)
            ax3.scatter(range(len(diffs)), diffs, c='r', label='相邻线段x坐标差')
            ax3.axhline(np.mean(diffs), color='b', linestyle='--', label='平均差值')
            ax3.set_xlabel('线段索引')
            ax3.set_ylabel('x坐标差(像素)')
            ax3.set_title(f'宽度均匀性评估 (标准差: {np.std(diffs):.2f}px)')
            ax3.legend()
    ...
    if matches:
        ...
        # 极坐标图
        ax4 = plt.subplot(2, 3, (4,6), projection='polar')
        ax4.plot(angles, radii, 'o-', label='深度分布')
        ax4.set_title(f'深度分布圆形度评估 (分数: {calculate_circularity(depths):.2f})')
        ax4.legend()
